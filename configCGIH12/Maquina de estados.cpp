...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		modelTemp= model = glm::rotate(model, glm::radians(dogRot), glm::vec3(0.0f, 0.0f, 0.001f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
void Animation() {
	...
	if (AnimDog)
	{	
		dogPos.z = sin(timer) * 15.0f;
		dogRot = sin(timer*150.0f)*2.0f;
		head = sin(timer*150.0f)*5.0f;
		tail = sin(timer * 150.0f) * 40.0f;
		FLegs = sin(timer * 150.0f) * 30.0f;
		RLegs = sin(timer*150.0f) * 30.0f;
