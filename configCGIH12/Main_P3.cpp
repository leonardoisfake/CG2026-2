...
#include <glm/glm.hpp> //Nueva bilbioteca
...
int main() {
 ...
 //     
	...
	glm::mat4 projection = glm::mat4(1); //Generar una matriz 4x4 rellena de unos.
	...
	while (!glfwWindowShouldClose(window)) //Bucle de renderizado
	{
		...
		model = glm::scale(model, glm::vec3(2.5f, 1.5f, 3.0f));
		model = glm::rotate(model, 3.0f, glm::vec3(1.0f, -0.5f, 1.5f));
		...
		model = glm::translate(model, glm::vec3(3.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(-0.5f, 5.5f, -1.5f));
		model = glm::rotate(model, -4.0f, glm::vec3(2.0f, -0.75f, -3.5f));
		...
		glBindVertexArray(0); //Esta función siempre tiene que ir hasta el final, porque limpia el VAO


		/*Ejercicio

		Dibujar dos cubos extra

		Escalar el primero en 1,1,1

		Uno a la izquierda, otro a la derecha, con diferente traslación, diferente escala y diferente rotación
		*/
